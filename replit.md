...
- **Checklist UX Improvements** (November 2025): Resolved race condition where checkbox toggles would auto-uncheck due to stale refetch payloads overwriting optimistic updates. Solution uses ref-based guard (lastServerChecklistRef) to track last applied server state, preventing useEffect from reverting local changes. Added "Marcar Todas" button to each checklist category header, enabling bulk completion while preserving existing item observations. Both features underwent architect review and stress-testing approval.
