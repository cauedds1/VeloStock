...
- **Vehicle Cost Currency Bug Fix** (November 2025): Corrigido bug crítico onde valores de custo eram multiplicados por 100 na primeira inserção devido a falta de conversão centavos↔reais. Backend armazena valores em centavos (integer). Frontend agora converte corretamente: `formatCurrency` divide por 100 ao exibir (VehicleCosts.tsx), `EditCostDialog` divide por 100 ao carregar valor para edição. Fluxo correto: adicionar 50 → ×100 → 5000 centavos → exibe R$ 50,00; editar 5000 centavos → ÷100 → campo "50.00" → ×100 → salva 5000 centavos → exibe R$ 50,00.
