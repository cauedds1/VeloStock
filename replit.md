...
- **UI Improvements** (November 2025): Removido horário de "Últimas Movimentações" em VehicleDetails (agora mostra apenas data dd/mm/yyyy). Sistema de "Observações Gerais da Loja" nos Relatórios refatorado para ter layout IDÊNTICO ao "Status do Checklist": barra de progresso "Taxa de Resolução", grid de 3 colunas com botão estático "Total de Observações" e botões clicáveis "Pendentes/Resolvidas" que abrem modal Dialog, calculando percentual de resolução (resolvidas/total).
- **Vehicle Cost System Simplification** (November 2025): Sistema de custos completamente simplificado removendo conversão centavos↔reais. Tipo de dados alterado de `integer` (centavos) para `numeric(10,2)` (reais diretos). Migration SQL executada para converter dados existentes (value/100). Removidas todas multiplicações e divisões por 100 no frontend/backend. Zod schema atualizado para aceitar number ou string com transform. Fluxo atual: usuário digita 150,99 → salva "150.99" no banco → exibe R$ 150,99. Drizzle retorna numeric como string, então usamos Number() nas somas. Arquivo de migration: `server/migrations/convert_costs_to_decimal.sql`.
