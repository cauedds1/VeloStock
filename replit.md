...
- **Database Connection Resilience** (November 2025): Centralized database connection logic via `server/config/database.ts` helper function (`getDatabaseUrl()`) to handle environment-specific DATABASE_URL resolution. In production (Replit Deploy), reads connection string from `/tmp/replitdb`; in development, falls back to `process.env.DATABASE_URL`. Eliminates production deployment failures where database credentials were previously unavailable. All database consumers (server/db.ts, drizzle.config.ts, migration scripts) now use this single helper for consistent behavior across environments. Includes logging of detected environment (dev vs prod) without exposing credentials.
